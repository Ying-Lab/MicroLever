...
"""Merge per-sample prediction CSVs from multiple cohorts and compute metrics.
...
Example:
  python scripts/eval_merge.py \
    --input Damman=results/damman/per_sample_results.csv \
    --input Other=results/other/per_sample_results.csv \
    --output-dir eval_real_merged
"""
...
parser.add_argument("--input", action="append", default=[], help="形如 NAME=path/to/per_sample_results.csv，可重复多次")
parser.add_argument(
    "--output-dir",
    type=str,
...
for item in args.input:
    if '=' not in item:
        raise ValueError('--input 需要形如 NAME=PATH')
    name, path = item.split('=', 1)
    RESULT_FILES[name.strip()] = path.strip()

if not RESULT_FILES:
    raise ValueError('请至少提供一个 --input NAME=PATH')
