...
CONFIG = {
    "paths": {
        # ---------- 模型 ----------
        "model_weights": "experiments/<run_name>/<timestamp>/models/best_*.pth",
        # "model_weights": "experiments/<run_name>/<timestamp>/models/best_*.pth",
        # "model_weights": "experiments/<run_name>/<timestamp>/models/best_*.pth",
        # "model_weights": "experiments/<run_name>/<timestamp>/models/best_*.pth",
        # "model_weights": "experiments/<run_name>/<timestamp>/models/best_*.pth",
        #"model_weights": "experiments/<run_name>/<timestamp>/models/best_*.pth",
        # "model_weights": "experiments/<run_name>/<timestamp>/models/best_*.pth",
        #mlp#"model_weights": "experiments/<run_name>/<timestamp>/models/best_*.pth",
        # "model_weights": "experiments/<run_name>/<timestamp>/models/best_*.pth",
        # "model_weights": "experiments/<run_name>/<timestamp>/models/best_*.pth",
        # ---------- 测试集 ----------
        # "h5_path": "data/real/FMT_real_data.h5",
        # "adjacency_csv": "data/adjacency/fake_A_53_dHOMO.csv",
        # "cdiff_label_csv": "data/real/Cdiff_labels_table.csv",
        # "donor_final_csv": "data/real/donor_final_abundance.csv",
    ...
    "output_dir": "results/real_test_run"
}
...
_parser = argparse.ArgumentParser("Real cohort prediction (CLR + spline sampling)")
_parser.add_argument("--config", default=None, help="JSON config, see configs/real_test_default.json")
_parser.add_argument("--model_weights", default=None, help="Override paths.model_weights")
_parser.add_argument("--output_dir", default=None, help="Override output_dir")
_args, _unknown = _parser.parse_known_args()

if _args.config:
    CONFIG = _load_config(_args.config)

if _args.model_weights:
    CONFIG.setdefault("paths", {})
    CONFIG["paths"]["model_weights"] = _args.model_weights
if _args.output_dir:
    CONFIG["output_dir"] = _args.output_dir
# ----------------------------------------------
